...
const PracticePage = {
  ...
  showResult(wrapper, q, selected, fromStorage) {
    ...
    // Show result banner + explanations
    const expHtml = q.options.filter(o => o.explanation).map(o => `
      <div class="explanation-item ${o.letter === q.correctAnswer ? 'correct' : ''}">
        <div class="explanation-letter">${o.letter}${o.letter === q.correctAnswer ? ' — CORRECT' : ''}</div>
        <div class="explanation-text">${Helpers.escHtml(o.explanation)}</div>
      </div>
    `).join('');
    ...
    revealed.innerHTML = `
      <div class="answer-result-banner ${isCorrect ? 'correct' : 'incorrect'}">
        ...
        <span class="answer-result-text">${isCorrect ? 'Correct!' : `Incorrect — Answer is ${q.correctAnswer}`}</span>
      </div>
      ${expHtml ? `<div class="explanations">${expHtml}</div>` : ''}
    `;
